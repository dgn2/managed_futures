...
The holding period return is the factor by which we multiply the starting value of the portfolio to get the ending value of a portfolio, given the monthly returns and weights of each component investment.
...
Using the participation ratio defined in the previous section, we see that `r round(participationRatio[1])` components make significant contributions to the first factor. This is in strong contrast to the other factors where the number of components making significant contributions is between `r participationRatioRange[1]` and `r participationRatioRange[2]`. 
