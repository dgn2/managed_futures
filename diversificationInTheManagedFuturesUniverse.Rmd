...
perturbFactorCorrelation <- function (factorIndex,eigenvalues,eigenvectors){
  ...
  shockFactors<-seq(from=0.1,to=5,by=0.1)
...
In this section we apply the theory outlined in the above section. In particular, we build a simple factor model of managed futures program returns and use it to create sensitivities that map the proportion of total variance in the investment universe to portfolio variation. This sensitivity owns us to monitor the time-varying proportion of variance explained by the first few statistical factors and instantly convert this measure into a portfolio variability. This is particularly useful during times of crisis when the importance of the first few factors increase significantly. 
...
portfolioV<-matrix(0,numberOfScenarios,2)
for (factorIndex in 1:numberOfFactors){
...
plot(varianceScenarios[,1]*100,portfolioV[,1]*100,
  ...
  main='Sensitivity Of Portfolio Variance to Changes in the Importance of the First Factor')
```
...
https://github.com/dgn2/managed_futures/find/master
