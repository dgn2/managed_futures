...
To determine how many statistical factors are needed to describe the correlations between investments, many methods have been proposed. There is no widespread agreement on an optimal approach. In this paper we select the most stra
...
Decompose the correlation matrix into statistical factors
...
knitr::kable(head(factorTable[factor_1_Index$ix,c(1,2,3)],10),digits=4,main='Smallest Factor 1 Loadings')
```
...
knitr::kable(tail(factorTable[factor_1_Index$ix,c(1,2,3)],10),digits=4,main='Smallest Factor 1 Loadings')
```
...
Using the participation ratio defined in the previous section, we see that `r round(participationRatio[1])` components make significant contributions to the factor. This is in strong contrast to the other factors where the number of components making significant contribtions is between `r participationRatioRange[1]` and `r participationRatioRange[2]`. 
...
I would like to acknowledge useful discussions with Paul Britton and Jean de Carufel of Apollo Systems Research Corporation. Both individuals have provided feedback about the ideas presented in this paper over the years. The views expressed in this paper do not reflect the views of my current employer, the Canadian Medical Protective Association (CMPA), or any of my previous employers, including Apollo Systems Research Corporation.
